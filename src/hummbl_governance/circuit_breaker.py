...
2. OPEN: Failure threshold exceeded. All calls rejected immediately.
3. HALF_OPEN: Recovery timeout elapsed. One probe call allowed.

If the probe succeeds, the breaker resets to CLOSED. If it fails,
the breaker returns to OPEN and the recovery timer restarts.
...
class CircuitBreakerState(Enum):
    ...
    CLOSED = auto()      # Normal: calls pass through, failures tracked
    OPEN = auto()        # Tripped: calls rejected immediately
    HALF_OPEN = auto()   # Testing: one probe call allowed
...
class CircuitBreaker:
    ...
    Tracks consecutive failures and trips open when the threshold is
    exceeded. After a configurable recovery timeout, allows a single
    probe call to test whether the underlying service has recovered.
    ...
    Args:
        ...
        on_state_change: Optional callback(old_state, new_state) invoked
            on every state transition. Callback errors are silently
            swallowed to prevent cascading failures.
    """
    ...
    def call(self, fn: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        ...
        In CLOSED state, the call proceeds normally. Failures increment
        the counter; when it reaches *failure_threshold* the breaker trips
        to OPEN.

        In OPEN state, calls are rejected with CircuitBreakerOpen unless
        the recovery timeout has elapsed, in which case the breaker
        transitions to HALF_OPEN and allows a single probe.

        In HALF_OPEN state, one probe call is in flight. If it succeeds,
        the breaker resets to CLOSED. If it fails, the breaker returns
        to OPEN.
        ...
        Raises:
            CircuitBreakerOpen: If the breaker is OPEN and the recovery
                timeout has not yet elapsed.
            Exception: Any exception raised by *fn* is re-raised after
    ...
    def reset(self) -> None:
        ...
        Clears failure count and last failure time. Success count is
        also reset. Fires the on_state_change callback if the state
        actually changed.
        """
        with self._lock:
    ...
    def _effective_state(self) -> CircuitBreakerState:
        """Compute the effective state, accounting for recovery timeout."""
