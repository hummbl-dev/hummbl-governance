...
import json
import tempfile
from hummbl_governance.reasoning import ReasoningEngine, Base120Model, ApplyResult

def test_reasoning_engine_loads_models():
    with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as tf:
        models = [
            {"code": "DE1", "name": "5 Whys", "transformation": "Decomposition", "definition": "Deep root cause analysis"},
            {"code": "S1", "name": "Systems Thinking", "transformation": "Systems", "definition": "Feedback loops"}
        ]
        json.dump(models, tf)
...
def test_generate_system_prompt():
    with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as tf:
        models = [
            {"code": "DE1", "name": "5 Whys", "transformation": "Decomposition", "definition": "Deep root cause analysis"},
            {"code": "P1", "name": "First Principles", "transformation": "Perspective", "definition": "Reducing to axioms"}
        ]
        json.dump(models, tf)
        tf_path = Path(tf.name)
    ...
    try:
        ...
        assert "why_5" in prompt # max(3, 2+3) = 5
...
def test_parse_llm_output():
    engine = ReasoningEngine() # Use default path for parsing test
    output = """
    ```json
    {
        "analysis": {"step1": "test"},
    ...
    ```
    """
    # DE1 model always exists in default data
