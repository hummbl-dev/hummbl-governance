...
def _nist_map_controls(governance_dir: str) -> dict:
    ...
    # Build CSF coverage
    csf_coverage: dict[str, list[dict]] = {}
    for subcat_id in [sc for func in NIST_CSF_CONTROLS.values() for sc in func["subcategories"]]:
        csf_coverage[subcat_id] = []

    # Build AI RMF coverage
    ai_rmf_coverage: dict[str, list[dict]] = {}
    for subcat_id in [sc for func in NIST_AI_RMF_CONTROLS.values() for sc in func["subcategories"]]:
        ai_rmf_coverage[subcat_id] = []
    ...
    for entry in entries:
        ...
        # Map to CSF
        for csf_sub in _TUPLE_TO_NIST_CSF.get(tuple_type, []):
            if csf_sub in csf_coverage:
                csf_coverage[csf_sub].append(base.copy())

        # Map to AI RMF
        for rmf_sub in _TUPLE_TO_AI_RMF.get(tuple_type, []):
            if rmf_sub in ai_rmf_coverage:
                ai_rmf_coverage[rmf_sub].append(base.copy())

    # Compute coverage stats
    csf_covered = sum(1 for v in csf_coverage.values() if v)
    csf_total = len(csf_coverage)
    ai_rmf_covered = sum(1 for v in ai_rmf_coverage.values() if v)
    ai_rmf_total = len(ai_rmf_coverage)

    # Identify gaps
    csf_gaps = [k for k, v in csf_coverage.items() if not v]
    ai_rmf_gaps = [k for k, v in ai_rmf_coverage.items() if not v]
    ...
    return {
        ...
        "nist_csf_2_0": {
            "coverage": f"{csf_covered}/{csf_total}",
            "coverage_pct": round(csf_covered / csf_total * 100, 1) if csf_total else 0,
            "controls": {k: {"evidence_count": len(v)} for k, v in csf_coverage.items()},
            "gaps": csf_gaps,
        },
        "nist_ai_rmf": {
            "coverage": f"{ai_rmf_covered}/{ai_rmf_total}",
            "coverage_pct": round(ai_rmf_covered / ai_rmf_total * 100, 1) if ai_rmf_total else 0,
            "controls": {k: {"evidence_count": len(v)} for k, v in ai_rmf_coverage.items()},
            "gaps": ai_rmf_gaps,
        },
    }
...
def _soc2_assess(governance_dir: str) -> dict:
    ...
    for category, criteria in SOC2_CRITERIA.items():
        criteria_evidence[category] = {}
        for crit_id in criteria:
            criteria_evidence[category][crit_id] = []

    # Map entries to SOC 2 criteria
    for entry in entries:
        tuple_type = entry.get("tuple_type", "")
        base = {
            "entry_id": entry.get("entry_id"),
    ...
    # Also use the ComplianceMapper for SOC2 report enrichment
    ...
    return {
        ...
        "overall_readiness": "READY" if overall_pct >= 80 else ("PARTIAL" if overall_pct >= 50 else "GAP"),
...
def _compliance_evidence_export(governance_dir: str, framework: str) -> dict:
    ...
    if framework in ("soc2", "all"):
        report = mapper.generate_soc2_report(days=90)
        package["evidence"]["soc2"] = {
    ...
    if framework in ("nist", "all"):
        package["evidence"]["nist"] = _nist_map_controls(governance_dir)

    if framework in ("iso", "all"):
        package["evidence"]["iso_crosswalk"] = _iso_crosswalk(governance_dir)

    if framework in ("owasp", "all"):
        report = mapper.generate_owasp_report(days=90)
        package["evidence"]["owasp_agentic"] = {
            "framework": report.framework,
            "generated_at": report.generated_at,
            "controls": report.controls,
            "summary": {ctrl: len(evs) for ctrl, evs in report.controls.items()},
    ...
    if framework == "all":
        # Add GDPR for completeness
        report = mapper.generate_gdpr_report(days=90)
        package["evidence"]["gdpr"] = {
            "framework": report.framework,
            "generated_at": report.generated_at,
            "controls": report.controls,
            "summary": {ctrl: len(evs) for ctrl, evs in report.controls.items()},
    ...
    total_evidence = 0
    for fw_data in package["evidence"].values():
        if "summary" in fw_data:
            total_evidence += sum(fw_data["summary"].values())
        elif "entries_analyzed" in fw_data:
            total_evidence += fw_data["entries_analyzed"]

    package["total_evidence_items"] = total_evidence
